...
{
  /// <summary>
  /// Class used from CodeProject article.  Originally written by Johnny J.: http://www.codeproject.com/Members/Johnny-J
  /// Original code under use of the CPOL: http://www.codeproject.com/info/cpol10.aspx
  /// 
  /// Enhanced methods for OS Version detection and OS information.
  /// </summary>
  public static class OSVersionInfoHelper
  {
    #region ENUMS
    ...
    static public ProcessorArchitecture ProcessorBits
    {
      get
      {
        ...
        try
        {
          OSVersionInfoNativeMethods.SYSTEM_INFO l_System_Info = new OSVersionInfoNativeMethods.SYSTEM_INFO();
          OSVersionInfoNativeMethods.GetNativeSystemInfo(ref l_System_Info);
    ...
    static public string Edition
    {
      get
      {
        ...
        OSVersionInfoNativeMethods.OSVERSIONINFOEX osVersionInfo = new OSVersionInfoNativeMethods.OSVERSIONINFOEX();
        osVersionInfo.dwOSVersionInfoSize = Marshal.SizeOf(typeof(OSVersionInfoNativeMethods.OSVERSIONINFOEX));

        if (OSVersionInfoNativeMethods.GetVersionEx(ref osVersionInfo))
        {
          ...
          else if (majorVersion == 5)
          {
            if (productType == VER_NT_WORKSTATION)
            {
              ...
              {
                if (OSVersionInfoNativeMethods.GetSystemMetrics(86) == 0) // 86 == SM_TABLETPC
          ...
          {
            int ed;
            if (OSVersionInfoNativeMethods.GetProductInfo(majorVersion, minorVersion,
    ...
    {
      IntPtr handle = OSVersionInfoNativeMethods.LoadLibrary("kernel32");

      if (handle != IntPtr.Zero)
      {
        IntPtr fnPtr = OSVersionInfoNativeMethods.GetProcAddress(handle, "IsWow64Process");
  ...
  internal static class OSVersionInfoNativeMethods
  {
    #region PINVOKE
